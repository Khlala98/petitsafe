      ...
      {structures.length > 1 ? (
        <div className="relative" ref={ref}>
          <button
            ...
            aria-label="Changer de structure"
          >
            {activeStructure?.structure.nom ?? "Structure"}
            ...
            <div className="absolute top-full left-0 mt-1 bg-white rounded-xl shadow-lg border border-gray-200 py-1 min-w-[200px] z-50">
              {structures.map((s) => (
                <button
                  key={s.structure_id}
                  onClick={() => {
                    onSwitchStructure(s.structure_id);
                    setOpen(false);
                  }}
                  className={`w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 ${
                    s.structure_id === activeStructureId
                      ? "text-petitsafe-primary font-medium"
                      : "text-gray-600"
                  }`}
                >
                  {s.structure.nom}
        ...
        <span className="text-sm font-medium text-gray-700">
          {activeStructure?.structure.nom ?? "PetitSafe"}
        </span>
      )}
